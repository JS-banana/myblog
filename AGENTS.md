...
Source of truth 在 `posts` 仓库：`/Users/sunss/myself/posts/blog/` 与 `/Users/sunss/myself/posts/wechat/`。
...
不要手动修改 `src/content/posts/` 下的同步文章。需要改标题、日期、正文或发布状态时，回到 `posts` 仓库源文件修改；需要改 slug、description、tags 时，先判断是否是一次性人工覆盖，默认仍交给同步脚本与 `src/content/.slug-cache.json` 固定。

本仓渲染入口：

- schema：`src/content.config.ts`
- 列表：`src/components/PostList.astro`
- 详情页：`src/pages/posts/[...slug].astro`
- 文章布局：`src/layouts/PostLayout.astro`
- SEO/RSS：`src/components/Head.astro`、`src/pages/rss.xml.ts`
...
pnpm typecheck
SITE_URL=https://blog.laifuyou.com pnpm build
```
